...
def build_sad_content(*, fig_caption_red: bool = True) -> list[ET.Element]:
    ...
    el.append(
        make_tbl(
            ["فرض", "اثر روی معماری", "اگر فرض نقض شود"],
            [
                ["پاسخ تأمین‌کنندگان ناهمگون و گاهی ناپایدار است.", "کنترل مهلت زمانی، کاهش سطح خدمت و تلاش‌مجدد کنترل‌شده ضروری است.", "ترکیب نتایج و تجربه کاربر باید بازطراحی شود (مثلاً نمایش تدریجی یا محدودسازی تأمین‌کننده)."],
                ["بازگشت بانک قابل تکرار است و ممکن است چند بار ارسال شود.", "کلید یکتایی عملیات و راستی‌آزمایی پرداخت الزامی است.", "ریسک صدور تکراری/ثبت تکراری بالا می‌رود و کنترل‌های مالی باید سخت‌گیرانه‌تر شود."],
                ["APIها به‌صورت عمومی در معرض اینترنت هستند.", "محدودیت نرخ، احراز هویت و ثبت ممیزی باید از ابتدا اعمال شود.", "در صورت نبود این کنترل‌ها، احتمال سوءاستفاده و اختلال بالا می‌رود."],
                ["در فاز نخست، استقرار به‌صورت تک‌استقرار ماژولار انجام می‌شود.", "سادگی عملیات و سرعت توسعه اولویت دارد، با حفظ مرزبندی دامنه‌ها.", "اگر نیاز به استقرار مستقل زودتر ایجاد شود، مرزبندی دامنه‌ها باید عملی‌تر و رابط‌ها دقیق‌تر شوند."],
            ],
            col_weights=[2, 2, 2],
        )
    )
    ...
    el.append(
        make_p(
            ...
            "به‌عنوان معیارهای پیشنهادی، این مقادیر در نظر گرفته می‌شود: زمان پاسخ جست‌وجو (صدک ۹۵) کمتر از ۳ ثانیه، نرخ خطای بازگشت بانک کمتر از ۱ درصد، "
    ...
    el.append(
        make_tbl(
            ["سنجه", "منبع داده", "نقطه اندازه‌گیری", "گزارش/داشبورد پیشنهادی"],
            [
    ...
    el.append(
        make_p(
            ...
            "در جدول زیر، ریسک‌های مهم، پیامدها و کنترل‌های پیشنهادی فهرست شده است تا در آزمون و عملیات به‌صورت فعال پیگیری شوند.",
            jc="both",
        )
    )
    el.append(
    ...
    el.append(
        make_p(
            ...
            "گزینه‌های موجود را از چند تأمین‌کننده دریافت و به‌صورت یکپارچه مشاهده کند.",
            jc="both",
        )
    )
    ...
    el.append(
        make_p(
            ...
            "بهتر است برای هر جست‌وجو زمان شروع و پایان، نام تأمین‌کننده و نتیجه (موفق/ناموفق) ثبت شود. "
    ...
    el.append(
        make_p(
            "برای کاهش پیچیدگی و افزایش توسعه‌پذیری، استفاده هدفمند از الگوهای طراحی توصیه می‌شود: "
    ...
    el.append(make_p("کارهای پس‌زمینه (اختیاری ولی پیشنهاد‌شده)", style="Heading2"))
    el.append(
        make_tbl(
            ["کار", "دلیل", "سیاست"],
            [
    ...
    el.append(
        make_tbl(
            ["ماژول/بسته", "توضیح", "فناوری‌های پیشنهادی", "وابستگی‌ها"],
            [
    ...
    el.append(make_p("قراردادهای API (نمونه)", style="Heading2"))
    el.append(
        make_tbl(
            ["مسیر خدمت", "توضیح"],
            [
    ...
    el.append(
        make_tbl(
            ["حوزه", "سنجه/هشدار", "آستانه پیشنهادی", "اقدام پیشنهادی"],
            [
    ...
    el.append(
        make_p(
            ...
            "این بخش، اهداف پیشنهادی را مشخص می‌کند تا در پیاده‌سازی و عملیات مبنا قرار گیرد.",
            jc="both",
        )
    )
    el.append(
        make_tbl(
            ["موضوع", "هدف پیشنهادی", "توضیح"],
            [
    ...
    el.append(
        make_p(
            "برای اینکه توصیه‌های کارایی قابل اجرا باشد، سنجه‌ها باید از محل‌های درست جمع‌آوری شوند. "
            "در این سند، اندازه‌گیری‌ها به گونه‌ای پیشنهاد می‌شوند که هم تجربه کاربر دیده شود و هم سهم هر وابستگی بیرونی قابل تفکیک باشد.",
            jc="both",
        )
    )
    el.append(
    ...
    el.append(make_p("تنظیمات پیشنهادی (اجرایی)", style="Heading2"))
    el.append(
        make_tbl(
            ["موضوع", "مقدار پیشنهادی", "توضیح"],
            [
    ...
    el.append(
        make_tbl(
            ["دارایی/حوزه", "تهدیدهای رایج", "کنترل‌های پیشنهادی"],
            [
    ...
    el.append(
        make_p(
            "برای اینکه کیفیت فقط در حد شعار نماند، چند هدف سطح خدمت به‌عنوان مبنا پیشنهاد می‌شود. "
            "این اهداف می‌توانند در فازهای بعدی به توافق عملیاتی تبدیل شوند، اما همین مقدار هم کمک می‌کند تیم‌ها معیار مشترک داشته باشند.",
            jc="both",
        )
    )
    el.append(
        make_tbl(
            ["موضوع", "هدف پیشنهادی", "روش ارزیابی"],
            [
    ...
    el.append(make_p("پیوست — چک‌لیست تحویل", style="Heading1"))
    el.append(
        make_p(
            "پیش از تحویل نهایی، چند مورد بازبینی می‌شود: کامل بودن تاریخچه بازبینی، قرار گرفتن همه نمودارها در محل‌های مربوط، "
            "درستی شماره‌گذاری بخش‌ها و شکل‌ها، و خوانایی جدول‌ها و تصاویر در خروجی ورد و پی‌دی‌اف.",
            jc="both",
        )
    )
