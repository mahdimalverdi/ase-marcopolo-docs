...
def ensure_toc_field(root: ET.Element, file_bytes: dict[str, bytes]) -> None:
    """
    Replace the template's static TOC text with a real TOC field.
    Note: page numbers are generated by Word/LibreOffice when fields are updated.
    """
    body = root.find("w:body", NS)
    if body is None:
        return
    ...
    # Remove static TOC lines until we reach the next repeated title paragraph.
    remove_from = toc_heading_idx + 1
    remove_to = remove_from
    for j in range(remove_from, len(children)):
        el = children[j]
        if el.tag == _qn("w:p") and _p_text(el) == "سند معماری نرم‌افزار":
            break
        remove_to = j + 1
    for el in children[remove_from:remove_to]:
        body.remove(el)

    # Insert TOC field right after the TOC heading.
    p = ET.Element(_qn("w:p"))
    pPr = ET.SubElement(p, _qn("w:pPr"))
    ET.SubElement(pPr, _qn("w:spacing"), {_qn("w:before"): "100", _qn("w:after"): "100"})
    ET.SubElement(pPr, _qn("w:jc"), {_qn("w:val"): "lowKashida"})
    _add_rtl_props(pPr)

    fld = ET.SubElement(p, _qn("w:fldSimple"), {_qn("w:instr"): 'TOC \\o "1-3" \\h \\z \\u'})
    r = ET.SubElement(fld, _qn("w:r"))
    _add_rtl_props(r)
    ET.SubElement(r, _qn("w:t")).text = " "

    body.insert(toc_heading_idx + 1, p)
...
def build_sad_content(*, fig_caption_red: bool = True) -> list[ET.Element]:
    ...
    el.append(
        make_p(
            "خارج از محدوده این سند، پیاده‌سازی سامانه‌های تأمین‌کنندگان، درگاه بانکی و سامانه بیرونی پشتیبانی است. "
            "این سامانه‌ها به‌عنوان سرویس‌های بیرونی مصرف می‌شوند و در معماری صرفاً از منظر قراردادهای تبادل داده، مدیریت خطا، مهلت زمانی پاسخ و امنیت ارتباطات تحلیل می‌شوند. "
            "زیرساخت پیامک/ایمیل نیز به‌عنوان سرویس بیرونی در نظر گرفته می‌شود.",
            jc="both",
        )
    )
