...
def build_sad_content(*, fig_caption_red: bool = True, include_appendix: bool = True) -> list[ET.Element]:
    el: list[ET.Element] = []
    ...
    if include_appendix:
        ...
        el.append(
            make_p(
                "این پیوست برای کنترل نهایی قبل از تحویل اضافه شده است تا در نسخه نهایی، موردی از قلم نیفتد.",
                jc="both",
            )
        )
        el.append(
            make_tbl(
                ["ردیف", "مورد کنترل", "وضعیت/یادداشت"],
                [
                    ["۱", "تاریخچه بازبینی تکمیل و تاریخ‌ها شمسی است.", ""],
                    ["۲", "نام سامانه، نسخه و نام اعضای گروه در صفحه اول، هدر و فوتر درست است.", ""],
                    ["۳", "فهرست مطالب بر اساس عنوان‌ها به‌روز شده و شماره صفحه‌ها درست نمایش داده می‌شود.", ""],
                    ["۴", "شماره‌گذاری بخش‌ها، شکل‌ها و جدول‌ها پیوسته و بدون تناقض است.", ""],
                    ["۵", "نمودارهای ویژوال پارادایم در جای درست قرار گرفته و خوانا هستند.", ""],
                    ["۶", "همه تصویرها/نمودارها در خروجی Word و PDF بدون خطا باز می‌شوند.", ""],
                    ["۷", "جدول‌ها از کادر صفحه بیرون نزده‌اند و در چاپ/خروجی خوانا هستند.", ""],
                    ["۸", "متن‌های توضیحی فقط در پاراگراف‌های پیوسته justify شده و سایر بخش‌ها دست‌نخورده است.", ""],
                ],
                col_weights=[1, 4, 2],
            )
        )
...
def main() -> int:
    ...
    parser.add_argument("--no-appendix", action="store_true", help="Do not include the delivery checklist appendix")
    ...
    for new_el in build_sad_content(fig_caption_red=not args.embed_images, include_appendix=not args.no_appendix):
